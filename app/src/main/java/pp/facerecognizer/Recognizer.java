...
public class Recognizer {
    ...
    public void setSaveFace(boolean saveFace) {
    ...
    List<Recognition> recognizeImage(Bitmap bitmap, Matrix matrix) {
        synchronized (this) {
            ...
            for (RectF rectF : faces) {
                ...
                FloatBuffer buffer = faceNet.getEmbeddings(bitmap, rect, this.saveFace);
    ...
    void updateData(int label, ContentResolver contentResolver, ArrayList<Uri> uris) throws Exception {
        synchronized (this) {
            ...
            for (Uri uri : uris) {
                ...
                faceNet.getEmbeddings(bitmap, rect, this.saveFace).get(emb_array);
                list.add(emb_array);
            }
